...
module.exports.RunAction = async function (client, msg, action) {
  ...
  switch (action.type) {
    ...
    case "Edit Message":
      ...
      break;
    case "Call API":
      await this.CallAPI_Handle(msg, client, parsedAction);
      break;
  }
};
...
module.exports.CallAPI_Handle = async function (msg, client, action) {
  ...
  try {
    const response = await fetch(action.url, {
      ...
      headers: {
        Authorization: `Bearer ${action.bearertoken}`,
        "Content-Type": "application/json",
      },
      body: action.requestbody ? JSON.stringify(JSON.parse(action.requestbody)) : undefined,
    });

    if (response.ok) {
      const responseData = await response.json();
      let tempVariableObject = cache[msg.guild.id].variables;
      tempVariableObject[action.responsevariablename] = responseData;
...
module.exports.CheckIfUserHasRole_Handle = function (msg, client, action) {
  let role = msg.guild.roles.cache.find((role) => role.name == action.role || role.id == action.role);
...
function getDescendantProp(obj, desc) {
  ...
  while (arr.length) {
    ...
    // Handle array indexing notation like [0]
